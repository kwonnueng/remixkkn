...
//				if(phoneNumArr[q].equals(phoneNum)){
				if(phoneNumArr[q].equals(sb.toString())){
					System.out.println("다시 입력받기");
					System.out.println("다시입력받기전 현재 phoneNum = "+sb.toString());
					i--;
					continue A;
				}
			}
//			phoneNumArr[i]=phoneNum;
			phoneNumArr[i]=sb.toString();
	...
	B:	for(int i=0; i<areaPhoneNumArr.length; i++){
		//지역번호 초기화
		selectNum=(int)(Math.random()*6);
		areaNum=areaStringArr[selectNum];	
			for(int j=0; j<7; j++){
				if(j==0||j==3){
					areaNum=areaNum+"-";
				}
		...
		System.out.println("070전화 만들기");
		
		String [] freeNumArr =new String [num];
		String freeNum="";
		A:for(int i=0; i<freeNumArr.length; i++){
			freeNum="070";
			for(int k=0;k<8; k++){
				if(k==0||k==4){
					freeNum=freeNum+"-";
				...
				randomNum=	(int)(Math.random()*10);
				freeNum=freeNum+Integer.toString(randomNum);
			
			}
		
			for(int q=0; q<=i-1; q++){
				if(freeNumArr[q].equals(freeNum)){	
					System.out.println("다시 입력받기");
					System.out.println("다시입력받기전 현재 freeNum = "+freeNum);
					i--;
					continue A;
				}
			}
			freeNumArr[i]=freeNum;
		}
			for(int i=0;i<freeNumArr.length;i++){
				System.out.println((i+1)+"번째 생성된 070번호 출력="+freeNumArr[i]);	
			}	
			System.out.println("--------070 전화번호 생성 완료--------");
			System.out.println();
